...
class arrayListType  
{ 
   friend ostream& operator<< (ostream& osObject, const arrayListType<elemType>& listOut)
   {
      for(int i=0; i < listOut.length; i++)
         osObject << listOut.list[i]<<" ";
      osObject <<endl;   
      return osObject;
   }
...
public:
    ...
    void retrieveAt(int location, arrayListType<elemType> &retItem) const;
...
void arrayListType<elemType>::retrieveAt(int location,
                                         arrayListType<elemType> &retItem) const
{
    if (location < 0 || location >= length)
...
template <class elemType>
const arrayListType<elemType>& arrayListType<elemType>::operator=(const arrayListType<elemType> &rightObject)
{
   if(this != rightObject)
   {
      delete []list;
